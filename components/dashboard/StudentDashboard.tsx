                                ...
                                {enrolledCourses.map((course) => {
                                    ...
                                    <div key={course.id} className="bg-slate-50 rounded-xl overflow-hidden hover:shadow-md transition-shadow group border border-slate-200">
                                        ...
                                        <div className="p-5">
                                            ...
                                            {/* El seminario ya se celebró: la tarjeta es la normal
                                                (grabaciones) y el grupo queda como acceso secundario. */}
                                            {isSeminario && (
                                                <a
                                                    href={SEMINARIO_WHATSAPP_GROUP_URL}
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                    className="mt-2 flex items-center justify-center gap-1.5 text-xs font-semibold text-[#1da851] hover:underline"
                                                >
                                                    Grupo de WhatsApp del seminario
                                                </a>
                                            )}
                                        </div>
                                    </div>
                                ...
                                {completedCourses.length > 0 ? (
                                    completedCourses.map((course) => (
                                        <div key={course.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-200">
                                            ...
                                            <a
                                                href={course.certificateUrl || (course.certificateId ? `/api/certificate/${course.certificateId}` : '#')}
                                                target="_blank"
                                                rel="noopener noreferrer"
